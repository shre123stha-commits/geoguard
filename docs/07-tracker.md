...
**Current task:** 1.10 (labelled set + metrics + threshold tuning)
...
- **Done (2026-09-19, Arena Agent):** Phase 0 (except manual 0.3/0.7), 1.1–1.9. pytest **61 passed**. End-to-end offline chain: `build_composites → preview_optical_change → preview_radar_change → fuse_detections` produces `detections.geojson` with confidence + score + `algorithm_version`. Owner has begun Wayback comparison (2020-12-16 vs 2023-06-13 releases at coarse zoom; marsh dry→wet visible and correctly *not* flagged) but has not yet given per-detection verdicts.
