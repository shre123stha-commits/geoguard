...
**Current phase:** Phase 3 done (auth, users, parcels API + login UI) — Phase 4 next (scans, worker, scheduler)
**Current task:** 4.1 `POST/GET /scans` on PostGIS (replacing the file-backed prototype route)
...
- **Done (2026-09-19, Arena Agent):** Phases 0 (except manual 0.7), 1, 2, 3. Owner ran migrations + seed + tests on Supabase (74 passed, D50). Phase 3 adds auth/users/parcels API + login UI; suite now **87 passed** (sandbox). `/parcels` is DB-backed; `/detections` and `/scans` are still `PrototypeStore` (file-backed) until Phases 4–5 replace them on the same paths.
- **Owner after pulling Phase 3:** `cd backend; pytest -q` → 87 passed. Start `uvicorn app.main:app --reload` + `npm run dev`, open http://localhost:5173 → sign in with `FIRST_ADMIN_EMAIL` / `FIRST_ADMIN_PASSWORD`, you will be asked to choose a new password, then the review map appears. Set a real `JWT_SECRET` (≥32 random chars) in `.env`.
