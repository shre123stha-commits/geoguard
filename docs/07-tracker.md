...
**Current task:** 1.6 (optical change mask + preview) — **investigate high candidate fraction first, see D30**
...
- **Done (2026-09-19, Arena Agent):** Phase 0 (except manual 0.3/0.7), 1.1–1.5. pytest **42 passed**. Index stats on composites: NDVI p50 0.30 → 0.26, BUI p50 −0.29 → −0.21, dBUI p50 +0.04 (AOI-wide positive shift), p95 +0.58. `data/cache/` is now **cleared before each commit** at the owner's request, so owner runs re-download on the hotspot.
