...
**Current task:** 1.5 (indices NDVI/NDBI/BUI + unit tests)
...
- **Done (2026-09-19, Arena Agent):** Phase 0 (except manual 0.3/0.7), 1.1–1.4. pytest **37 passed**, ruff/mypy clean. Real composites built in the sandbox in 81 s (all 26 S2 + 12 S1 scenes; cache now ~9 MB): baseline B04/B08/B11 p50 = 0.129/0.253/0.255, current 0.135/0.249/0.265 → same scale after offset fix; VV dB p50 −5.7 vs −5.2. Composites saved to `data/composites/{baseline,current}_{s2,s1}.npz` (bands, count, scene_ids).
